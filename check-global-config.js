...
const options = {
    ...
    port: 443,
    path: '/rest/v1/app_settings?key=eq.global_config&select=key,value',
    method: 'GET',
    headers: {
...
const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
        if (res.statusCode !== 200) {
            console.error('Failed to fetch global_config:', res.statusCode, data);
            return;
        }
        const rows = JSON.parse(data);
        if (rows.length === 0) {
            console.log('No settings found with key "global_config".');
            return;
        }
