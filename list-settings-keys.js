...
const options = {
    ...
    port: 443,
    path: '/rest/v1/app_settings?select=key,updated_at',
    method: 'GET',
    headers: {
...
const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
        if (res.statusCode !== 200) {
            console.error('Failed to fetch keys:', res.statusCode, data);
            return;
        }
        const rows = JSON.parse(data);
        console.log('--- ALL KEYS IN app_settings ---');
        rows.forEach(r => console.log(`Key: ${r.key} | Updated: ${r.updated_at}`));
    });
});
