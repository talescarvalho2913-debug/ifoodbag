...
const options = {
    ...
    port: 443,
    path: '/rest/v1/event_dispatch_queue?channel=eq.pixel&order=created_at.desc&limit=20',
    method: 'GET',
    headers: {
...
const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
        ...
        console.log(`--- PIXEL DISPATCH QUEUE (${rows.length} records) ---`);
        rows.forEach(r => {
            console.log(`[${r.created_at}] Event: ${r.event_name} | Result: ${r.result} | Reason: ${r.fail_reason}`);
        });
    });
});
