...
async function requestCreateTransaction(config = {}, payload = {}) {
    ...
    const tracking = payload.tracking || {
        utm_source: toText(metadata.utm_source || payload.utm_source),
        utm_medium: toText(metadata.utm_medium || payload.utm_medium),
        utm_campaign: toText(metadata.utm_campaign || payload.utm_campaign),
        utm_term: toText(metadata.utm_term || payload.utm_term),
        utm_content: toText(metadata.utm_content || payload.utm_content),
        src: toText(metadata.src || payload.src),
        sck: toText(metadata.sck || payload.sck),
        fbclid: toText(metadata.fbclid || payload.fbclid),
        gclid: toText(metadata.gclid || payload.gclid),
        ttclid: toText(metadata.ttclid || payload.ttclid)
    };
    ...
    const atomopayPayload = {
        ...
        payment_method: 'pix',
        postback_url: toText(payload.postback_url),
        tracking,
        metadata,
        cart: [
            {
                title: payload.rewardName || 'Bag do iFood',
