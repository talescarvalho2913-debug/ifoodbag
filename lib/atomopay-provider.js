...
async function requestCreateTransaction(config = {}, payload = {}) {
    ...
    // Prepare Atomopay payload
    const totalAmount = Number(payload.amount || 0);
    const amountInCents = Math.round(totalAmount * 100);
    const offerHash = toText(config.offerHash) || '6qiwtppuic';
    ...
    const atomopayPayload = {
        ...
        customer: {
            name: toText(payload.personal?.name),
            email: toText(payload.personal?.email),
            document: toText(payload.personal?.cpf || payload.personal?.document),
            phone: toText(payload.personal?.phone)
        }
    };
