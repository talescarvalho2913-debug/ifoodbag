...
const DEFAULT_ACTIVE_GATEWAY = 'sunize';
...
function buildAtomopayConfig(raw = {}) {
    ...
    const apiToken = toText(raw.apiToken) || toText(raw.apiKey) || toText(process.env.ATOMOPAY_API_TOKEN) || toText(process.env.ATOMOPAY_API_KEY);
    const offerHash = toText(raw.offerHash) || toText(process.env.ATOMOPAY_OFFER_HASH);
