...
function buildUtmfyOrder(eventName, payload = {}, cfg = {}) {
    ...
    let rawCreatedAt = payload.createdAt || payload.pixCreatedAt || payload.data_registro || payload.data_transacao || Date.now();
    if (status === 'paid') {
        rawCreatedAt = payload.approvedDate || payload.approvedAt || rawCreatedAt || Date.now();
    }

    // Safeguard: UTMify rejects dates older than 24 hours (SCHEMA_VALIDATION_FAILED)
    const oneDayAgo = Date.now() - 24 * 60 * 60 * 1000;
    const rawCreatedAtMs = new Date(rawCreatedAt).getTime();
    if (!Number.isNaN(rawCreatedAtMs) && rawCreatedAtMs < oneDayAgo) {
        rawCreatedAt = Date.now();
    }

    const createdAt = toIsoUtc(rawCreatedAt);
    const approvedDate = status === 'paid' ? toIsoUtc(payload.approvedDate || payload.approvedAt || rawCreatedAt || Date.now()) : null;
    ...
    const utm_source = tracking.utm_source || utm.utm_source || payload.utm_source || payload.src || '';
    const utm_campaign = tracking.utm_campaign || utm.utm_campaign || payload.utm_campaign || payload.campaign || '';
    const utm_medium = tracking.utm_medium || utm.utm_medium || payload.utm_medium || '';
    const utm_content = tracking.utm_content || utm.utm_content || payload.utm_content || payload.adset || '';
    const utm_term = tracking.utm_term || utm.utm_term || payload.utm_term || '';
    const fbclid = tracking.fbclid || utm.fbclid || payload.fbclid || '';
    const gclid = tracking.gclid || utm.gclid || payload.gclid || '';
    const ttclid = tracking.ttclid || utm.ttclid || payload.ttclid || '';
    const src = tracking.src || utm.src || payload.src || '';
    const sck = tracking.sck || utm.sck || payload.sck || '';
    ...
    return {
        ...
        trackingParameters: {
            src: toNullable(src),
            sck: toNullable(sck),
            utm_source: toNullable(utm_source),
            utm_campaign: toNullable(utm_campaign),
            utm_medium: toNullable(utm_medium),
            utm_content: toNullable(utm_content),
            utm_term: toNullable(utm_term),
            fbclid: toNullable(fbclid),
            gclid: toNullable(gclid),
            ttclid: toNullable(ttclid)
        },
